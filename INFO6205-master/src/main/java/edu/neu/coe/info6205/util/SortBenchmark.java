...
public class SortBenchmark {
    ...
    void benchmarkStringSortersInstrumented(String[] words, int nWords, int nRuns) {
        ...
        if (isConfigBenchmarkStringSorter("mergesort")) {
    			Config x = config.copy("instrumenting", "insurance", "false").copy("instrumenting", "noCopy", "false");
            runStringSortBenchmark(words, nWords, nRuns, new MergeSortBasic<>(nWords, x), timeLoggersLinearithmic);
        
        		Config x1 = config.copy("instrumenting", "insurance", "true").copy("instrumenting", "noCopy", "false");
            runStringSortBenchmark(words, nWords, nRuns, new MergeSortBasic<>(nWords, x1), timeLoggersLinearithmic);

        		Config x2 = config.copy("instrumenting", "insurance", "false").copy("instrumenting", "noCopy", "true");
            runStringSortBenchmark(words, nWords, nRuns, new MergeSortBasic<>(nWords, x2), timeLoggersLinearithmic);

        		Config x3 = config.copy("instrumenting", "insurance", "true").copy("instrumenting", "noCopy", "true");
            runStringSortBenchmark(words, nWords, nRuns, new MergeSortBasic<>(nWords, x3), timeLoggersLinearithmic);
        }
