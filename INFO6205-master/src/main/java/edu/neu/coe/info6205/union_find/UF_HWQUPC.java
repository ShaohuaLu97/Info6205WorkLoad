...
//package edu.neu.coe.info6205.util.Benchmark_Timer.java;
...
public class UF_HWQUPC implements UF {
    ...
    private void updateHeight(int i, int j) {
//        height[p] += height[x];
        if (i == j)
            return;
    ...
    private void mergeComponents(int i, int j) {
    	...
    	// height of i is smaller than height of j
    ...
    public static int count(Random rd,int i) {
    ...
    UF_HWQUPC uf = new UF_HWQUPC(i);
	   while(count>=0) {
		   ...
		   int numberi = rd.nextInt(i); // storing random integers in an array
		   int numberj = rd.nextInt(i);
	      if (!uf.connected(numberi, numberj)) {
	    	  uf.union(numberi, numberj);
	    	  count ++;
	      }
    	  else {
    		  count ++;
    ...
    public static void main(String[] args) throws IOException {
    	int p = 0 ;
    	System.out.println("Input a number as n, enter quit to end");
        ...
        while (true) { 
            ...
            int res_m = 0;
        	res_m = count(rd, Integer.parseInt(readLine));
            long endTime = System.nanoTime();
            long duration = (endTime - startTime);
        	System.out.println(res_m);
            System.out.println("Time duration: " + duration / 10000);
